...
const ChooseServiceToUnpublish = () => {
  ...
  const publishService = async (serviceId: number) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Error al despublicar el servicio');
      }
  
      toaster.create({
        title: "Servicio despublicado",
        description: "El servicio se despublicó correctamente.",
        type: "success",
        duration: 2000,
      });
    ...
    window.location.reload();
      ...
      console.error("Error al despublicar servicio:", err);
  ...
  return (
    <Box minH="100vh" w="100vw" bg="white" px={{ base: 4, md: 10 }} py={{ base: 4, md: 6 }}>
      ...
      <Flex direction="column" mt={2} mb={2} gap={2}>
        <Text color="#fd6193" fontWeight="bold" fontSize="2xl">Seleccione el servicio a despublicar</Text>
      </Flex>
...
export default ChooseServiceToUnpublish;